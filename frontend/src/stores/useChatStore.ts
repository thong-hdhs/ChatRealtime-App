...
export const useChatStore = create<ChatState>()(
    persist(
        (set, get) => ({
            ...
            updateConversationUserInfo: (userId, displayName, avatarUrl) => {
                set((state) => {
                    ...
                    const updatedConversations = state.conversations.map(
                        (c) => {
                            ...
                            // Cập nhật participants nếu có (rename fix)
                            if (c.participants) {
                                updated.participants = c.participants.map(
                                    (m) => {
                            ...
                            // Cập nhật sender của last message
                            if (
                                c.lastMessage &&
                                c.lastMessage.sender?._id?.toString() ===
                                    userIdStr
                            ) {
                                updated.lastMessage = {
                                    ...c.lastMessage,
                    ...
                    const updatedMessages = Object.entries(
                        ...
                        (acc: typeof state.messages, [convoId, msgData]) => {
                            if (!msgData || !msgData.items) {
                                acc[convoId] = msgData;
                                return acc;
                            }
            ...
            markAsSeen: async () => {
                try {
                    ...
                    console.error(
                        "Lỗi xảy ra khi gọi markAsSeen trong store",
                        error,
                    );
                }
            },
